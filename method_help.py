...
_REQUIRED_IMAGE_ZOOM_REVISION = 2026082703
...
# 2026-08-26 상하님 지시 — "'가려면 클릭' 이 텍스트 지워라. 좌우 폭 줄이면
# '이 테마 설명'과 같은 라인 선상에 옆으로 나란히 할 수 있다."
# 화살표만 남겨 '저기로 간다'는 뜻은 지킨다. 짧아져서 폰에서도 한 줄에 들어간다.
CROSS_PAGES = {
...
#
# 그림을 바꾸려면 assets/의 같은 이름 파일을 덮어쓰면 된다. 코드는 안 고쳐도 된다.
# 맨 위 만화 한 장 (2026-08-27 상하님이 주신 것으로 갈아 끼웠다).
# **두 칸 · 여덟 줄이다**(1080×1920). 넉 줄이던 옛 그림은 폰(390px)에서 한 칸이
# 81px이라 글씨를 못 읽었다. 두 칸이면 한 칸이 **162px**로 두 배가 되고, 세로로
# 길어져 손가락으로 내려 보시게 된다.
...
성적을 오히려 나쁘게 한 경우가 많았습니다. <b>손절과 파는 시점은 상하님이 정하십니다.</b>
</div>
</div>
"""
...
<li><b>손절 규칙은 없습니다.</b> 손절과 파는 시점은 상하님이 정하십니다.</li>
