...
def get_pass_candidates(
    ...
    *,
    ...
    테마별로 따로 보면 어느 테마의 몇 위가 지금 실제로 살 자리인지 한눈에 안 들어온다는
    지적(2026-07-22)에 따라, 상위 테마들의 종목을 한 번에 심사해 통과분만 추린다.
    ...
    def _scan(theme):
        ...
        if result.get("ok"):
            for leader in result["rows"]:
                ...
                item = {**leader, "theme_name": theme["name"], "theme_row": theme}
                if plan.get("recommendation") == "조건부 후보":
                    passed.append(item)
                elif plan.get("state") in {"돌파 확인", "눌림목 대기"}:
    ...
    def dedupe(items):
        """같은 종목이 여러 테마에 속하면 점수가 가장 높은 것 하나만 남긴다
        (예: 기아는 '자동차 대표주'와 '피지컬 AI' 양쪽에 있다)."""
        best = {}
        for item in items:
            code = item["code"]
            if code not in best or by_score(item) > by_score(best[code]):
                best[code] = item
        return sorted(best.values(), key=by_score, reverse=True)
    ...
    blocked_reason = None
    if not passed_rows and waiting_rows:
        if market_score < 50:
            blocked_reason = (
                f"시장 조건점수가 {market_score:.0f}점(50점 미만)이라 매수 게이트가 닫혀 있습니다. "
                "아래는 가격 셋업만 완성된 대기 종목입니다."
            )
        else:
            blocked_reason = (
                "테마 강도 또는 종목 점수가 기준에 못 미쳐 통과 종목이 없습니다. "
                "아래는 가격 셋업만 완성된 대기 종목입니다."
            )
