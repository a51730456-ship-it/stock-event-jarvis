...
st.markdown(
    ...
    /* ── 왼쪽 메뉴를 통째로 없앤다 (2026-08-09 상하님 선택) ────────────────────
       한국테마↔미국테마는 맨 위 '가려면 클릭' 단추로 오간다. 나머지 다섯 화면은
...
_REQUIRED_GAUGE_UI_REVISION = 2026080910
...
_REQUIRED_METHOD_HELP_REVISION = 2026082704
...
def _render_stock_detail(theme_row: dict, leader: dict, market: dict, top_candidates: list[dict],
    ...
    # 만점은 모듈에서 읽는다 — 여기에 숫자를 박아 두면 배점을 고칠 때 표만 옛 숫자로
    # 남는다(2026-08-07에 실제로 그럴 뻔했다). 차례는 score_parts와 같아야 한다.
    weights = getattr(j4data, "SCORE_WEIGHTS", None)
    factor_max = leader.get("factor_max") or (
        [weights[k] for k in ("relative", "high", "trend", "liquidity", "risk", "flow")]
        if weights else [20, 15, 20, 15, 10, 20])
...
**2026-08-07에 배점을 실측으로 다시 짰습니다.** 12년치 2,272종목을 창 2·3·4년씩
한 달 간격으로 밀며 재 봤더니, 옛 배점 100점 중 **40점이 거꾸로**였습니다.

| 항목 | 옛 배점 | 새 배점 | 창 2·3·4년 성적 |
|---|---|---|---|
| 52주 신고가 위치 | 15 | **35** | 88 · 95 · 100% ○ |
| 변동성 안정 | 10 | **25** | 86 · 73 · 77% ○ |
| 유동성(거래대금) | 15 | **20** | 69 · 64 · 57% △ (합격선 65% 미달, 거꾸로는 아님) |
| 수급(외국인+기관) | 20 | **20** | 못 쟀습니다 — 과거 수급 자료가 없습니다 |
| 추세(20·50·200일선) | 20 | **0** | 17 · 17 · 6% ✗ 거꾸로 |
| 테마 대비 상대강도 | 20 | **0** | 5 · 10 · 0% ✗ 거꾸로 |

이동평균선 위에 있다고 점수를 주면 한국에서는 **오히려 손해**였습니다. 50일선 위인
종목은 107개 창 중 11개에서만 이겼습니다. 그래서 두 항목은 0점으로 뺐습니다.
계산은 남겨 뒀으니 나중에 다시 재서 되살릴 수 있습니다.

(옛 기록) 국내 대형주 상당수가 52주 고가 대비 −30~−45% 구간이라 미국 기준(−25%~0)에서는
전 종목이 0점이 됐습니다. 신고가 항목의 범위가 −45~0인 것은 그 때문이고 지금도 같습니다.
            """
        )
...
def _render_my_stock_panel(market: dict) -> None:
    ...
    st.caption(
        ...
        "다만 2026-08-07 실측에서 상대강도는 **거꾸로**로 나와 배점이 0점이 됐으므로, "
        "빠져도 점수 차이는 없습니다."
    )
    _render_stock_detail(
...
def _render_rulebook_finder(result: dict, mode: str) -> None:
    ...
    if show_help and breakout:
        ...
        floor = float(rule.get("liquidity_floor") or 0) / 1e8
        st.markdown(
            "<div class='j4-pull-guide'>"
            f"<b>찾는 기준</b> — 하루 평균 거래대금 <b>{floor:,.0f}억 이상</b>인 종목 중, "
            f"52주 신고가를 찍고 <b>{wait_min}~{wait_max}거래일</b>이 지난 뒤 "
            f"그 고점에서 <b>{abs(drop_high):.0f}~{abs(drop_low):.0f}%</b> 내려온 것입니다. "
            f"사면 <b>{rule.get('hold_days')}거래일</b>(약 1년) 들고 갑니다. "
            "이동평균은 보지 않습니다.<br>"
            f"<b>한국 자료로 잰 성적</b>({span}) — 100번 중 "
            f"<b>{rule.get('win_rate')}번</b>({rule.get('sample'):,}건) · "
            f"가운데 값 <b>{rule.get('median_return')}%</b><br>"
            "<b>아무 종목이나 사서 같은 기간 들고 있었으면</b> — 100번 중 "
            f"{rule.get('base_win_rate')}번 · 가운데 값 {rule.get('base_median_return')}%<br>"
            f"<b class='j4-green'>기준선보다 나았던 해 {rule.get('years_total')}년 중 "
            f"{rule.get('years_better')}년 · 3년 창 {rule.get('windows_won')}개</b> — "
            "창을 한 달씩 밀며 100번 넘게 재도 한 번도 지지 않았습니다.<br>"
            "<b class='j4-down'>다만 가운데 값이 마이너스입니다.</b> 아무 종목이나 사면 "
            ...
            "<b>재 보고 뺀 것</b> — 거래대금이 평소 위에 며칠 연속인가(새 그물에서는 "
        ...
        st.markdown(
            ...
            f"<b class='j4-down'>다만 이 신호는 12년 동안 {events}번 났습니다</b> — "
            ...
            "(가장 나쁜 창 -43.7%p).<br>"
            "<b>재 보고 뺀 것</b> — 낙폭 갈래(그물로 이미 한 번 썼습니다), "
            "거래대금 평소 위 연속(새 그물에서는 안 갈렸습니다), "
            "외국인+기관 동반(재 보니 거꾸로였습니다 — 같이 산 날이 많을수록 나빴습니다).</div>"
...
def _render_pullback_finder() -> None:
    ...
    with st.expander("눌림목 종목 찾기 설명 보기", expanded=False):
        # 2026-08-07에 이 화면의 조건점수를 상승장·급락과 **같은 방법**으로 처음
        # 재 봤다(research/kr_cond_check.py). 결과가 나빠서 그대로 적어 둔다 —
        # 감추면 화면이 검증되지 않은 점수를 검증된 것처럼 말하게 된다.
        st.markdown(
            "<div class='j4-pull-guide' style='border-left:4px solid #ff5b5b;"
            " padding-left:.8rem'>"
            "<b class='j4-down'>⚠ 2026-08-07에 이 점수의 배점을 다시 짰습니다</b> — "
            "12년치(테마 2,272종목)를 창 2·3·4년씩 밀며 처음 재 봤더니 "
            "옛 배점 100점 중 <b>40점이 거꾸로</b>였습니다. "
            "이동평균 위(20점)는 창 107개 중 11개에서만 이겼고, "
            "20일 상대강도(20점)는 0~10%였습니다. <b>둘 다 0점으로 뺐습니다.</b> "
            "대신 검증에 통과한 <b>신고가에 가까운가</b>(88·95·100%)를 15→35점, "
            "<b>변동성 안정</b>(86·73·77%)을 10→25점으로 올렸습니다.<br>"
            "<b>아직 남은 약점</b> — 수급 20점은 과거 자료가 없어 못 쟀고, "
            "거래대금 20점은 합격선(65%)을 못 넘었습니다. 그리고 이 화면의 종목은 "
            "정의상 전부 신고가 근처라 35점이 거의 다 만점으로 붙습니다. "
            "<u>점수 순서를 매수 순서로 읽지 마십시오.</u> 이 화면은 ‘이걸 사라’가 "
            "아니라 <b>‘지금 사면 얼마나 위험한가’</b>를 재는 것입니다.</div>",
            unsafe_allow_html=True,
        )
        st.markdown(
            ...
            "신고가 최근성 25 + 20일선 근접 25 + 조정 깊이 25 + 수급 15을 더한 "
            f"<b>최대 {j4data.PULLBACK_SCORE_MAX:.0f}점</b>입니다. "
            "여기 있던 <b>장기 추세 20점(50·200일선 위)도 2026-08-07에 뺐습니다</b> — "
            "위 배점표의 추세와 같은 것을 재는데 실측에서 거꾸로였습니다."
            "</div>",
        unsafe_allow_html=True,
        )
...
def _render_method_tab() -> None:
    ...
    st.markdown(
        ...
        3. **대장주 품질(6개 항목)** — **52주 신고가 위치 35 + 변동성 안정 25** +
           유동성 20 + 수급(외국인·기관) 20 + 추세 0 + 테마 대비 상대강도 0 = 100점.
           2026-08-07 실측에서 추세와 상대강도가 거꾸로로 나와 0점으로 뺐고, 그만큼을
           검증에 통과한 앞의 둘로 옮겼습니다.
