...
US_THEMES = (
    ...
    # SATS 는 뺐다 (2026-09-07 상하님 지시). **상장폐지되어 시세가 아예 안 온다** —
    # 받을 때마다 실패하고 다시 시도해 명부 한 벌 받는 데 0.6초를 더 썼다(실측).
    # 계산에는 원래도 안 들어갔다(자료가 없는 종목은 건너뛴다). 그래서 이 테마의
    # 점수·등수·강한 종목 비율은 빼기 전과 같아야 한다 — 다르면 그것이 버그다.
    {"name": "우주·위성", "etf": "UFO", "alt_etf": "ARKX", "stocks": ("RKLB", "ASTS", "LUNR", "RDW", "PL", "IRDM")},
...
def _compute_theme_rankings() -> dict:
    ...
    for theme in US_THEMES:
        ...
        rows.append({
            ...
            # 테마 ETF 의 **절대** 6개월 수익률 (2026-09-05 상하님 지시로 화면에
            # 적으려고 담는다). 「6개월 시장대비」는 나스닥을 뺀 값이라, 그것만
...
def prepare_breakout_scan() -> None:
    ...
    **「방금 해 뒀다」는 시계가 아니라 기억으로 판단한다** (2026-09-18 온라인
    실측으로 잡은 흠). 뒤 일꾼은 `_BREAKOUT_WARM["at"]` 시각을 보고 5분 안이면
    건너뛴다. 그런데 `clear_runtime_cache()`(맨 위 ↻)는 **기억만 비우고 그 시각은
    안 지운다.** 그래서 기억이 텅 비었는데도 "방금 해 뒀다"며 건너뛰고, 단추가
    제 손으로 200종목을 계산했다 — 온라인에서 고친 뒤에도 첫 클릭이 1.99초로
    그대로였던 까닭이 이것이다(고치기 전 1.97초).

    `_memo_ok` 가 이미 그 판단을 한다 — 기억이 5분 안이면 **그 자리에서** 돌려주고
    없으면 만든다. 그러니 시각을 따로 보지 않는다. 기억도 없고 공책도 비었으면
    `_download_cache_only` 가 곧바로 빈손으로 돌아오므로 판마다 불러도 값이 없다.

    자물쇠(`on`)는 뒤 일꾼과 **같은 것**을 쓴다 — 둘이 같은 계산을 두 벌 돌리지
    않게 한다. 시각(`at`)은 성공했을 때 적어 둔다 — 뒤 일꾼 쪽이 그것을 본다.
    """
    with _BREAKOUT_WARM_LOCK:
        if _BREAKOUT_WARM["on"]:
    ...
    for symbol in symbols:
        ...
        if len(points) >= 2 and base:
            ...
            result[symbol] = {
                ...
                "daily_ohlc": _ohlc_rows(closes, len(closes)),
            }
    return result
...
def get_etf_sparklines(
    ...
    for symbol in wanted:
        ...
        if len(day_points) >= 2:
            ...
            # 봉차트 재료(ohlc)도 싣는다(2026-09-24 상하님 — "일봉은 봉차트로"). 이미 받은 일봉이다.
            pair["daily"] = {"points": day_points, "base": day_points[0],
                             "ohlc": _ohlc_rows(closes, daily_sessions)}
        if pair:
            result[symbol] = pair
    return result


def _ohlc_rows(frame, limit: int) -> list:
    """봉차트 재료 — 마지막 limit 개 일봉의 [시가, 고가, 저가, 종가] (2026-09-24). 빈 값이 있는 날은 뺀다.

    이미 받아 둔 일봉 표에서 꺼내기만 한다 — 새로 받지 않는다. 칸이 없으면 빈 목록(그러면 화면은
    예전 선 그림으로 간다).
    """
    try:
        sub = frame[["Open", "High", "Low", "Close"]].dropna().astype(float).tail(int(limit))
    except Exception:
        return []
    return [[round(o, 4), round(h, 4), round(l, 4), round(c, 4)]
            for o, h, l, c in sub.itertuples(index=False)]
