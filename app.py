...
_ALL_DEST_OPTIONS = [
    ...
    "선행감지 (자비스5·실험)",
    "종가관찰 (자비스6·연습)",
    # 새 디자인 미국테마 (2026-09-03 상하님 지시). 옛 미국테마는 위에 그대로 둔다 —
    # 둘을 나란히 놓고 견주시는 것이 이 화면을 만든 까닭이다.
    "자비스6 미국테마 (새 디자인)",
    "자비스7 미국테마",
    "자비스8 미국주식 연구실",
]
...
_DEST_PAGES = {
    ...
    "자비스6 미국테마": "pages/6_자비스6_미국테마.py",
    "자비스7 미국테마": "pages/7_자비스7.py",
    "자비스8 미국주식 연구실": "pages/8_자비스8.py",
}
...
if (st.query_params.get("page") != _JARVIS1_URL_MARK
    ...
    # 게스트 목록은 짧아서 위 '앞 3개·뒤 2개 감추기'가 걸리면 안 된다 — 상자를 따로 쓴다.
