...
def test_daily_charts_are_candles_with_whole_number_coordinates():
    ...
    # 두 자리가 봉차트를 쓴다 — 종목 상세·대장주 비교(일봉). 시장분석 지수 칸 「6개월」은 **원래 선 그림**으로
    # 되돌렸다(2026-09-25 상하님 — "봉차트 말고 원래 차트로").
    ...
    swap = source.split("def _index_chart_swap(")[1].split("\ndef ")[0]
    assert "_candle_svg" not in swap and 'spark.get("daily_ohlc")' not in swap, "지수 칸 6개월이 봉차트다"
    assert '{"points": daily_points, "base": spark.get("daily_base")}' in swap
