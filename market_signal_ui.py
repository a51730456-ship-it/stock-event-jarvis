...
def run_us_market_signal_check(force_refresh=False):
    ...
    # *"장 시작 시 열고, 장 종료 시 당일 마감하고, 그게 다음날 장 시작 시 전일로
    # 넘어가면 되지 않냐?"*).
    #
    # 여태는 **마감 종이 울리는 순간** 위 칸이 다음 장으로 넘어갔다(2026-08-26 지시).
    # 그런데 다음 장은 열일곱 시간 뒤에나 열린다. 그 사이 내내 위 칸에 담을 것이
    # 없어서 「자료 부족」으로 섰고, 바늘도 없었다 — 상하님이 보시는 한국 아침이
    # 바로 그 시간대다. 열세 가지 중 VIX·미국 10년물·S&P500·나스닥은 미국장이
    # 닫히면 **새 값이 아예 안 생기기** 때문이다.
    #
    # 넘기는 때를 **마감**에서 **다음 개장**으로 옮긴다. 그러면 한국 낮에는 위 칸이
    # 방금 끝난 장의 확정 판정(일봉으로 잰 것)을 그대로 들고 있고, 아래 칸은 그
    # 하루 앞 장이 된다. 둘 다 이미 아래에서 만들어 두던 것이라 **새로 받아 오는
    # 자료가 없다** — 화면 위 지수 칸이 쓰는 그 마감값과 같은 것이다.
    # 미국장이 열리면 지금까지와 똑같이 위 칸이 오늘 장(장중 실시간)으로 바뀐다.
    live_usable = session_open and bool(quotes) and getattr(live_result, "verdict", None) is not None
...
def render_us_market_signal_card(*, foldable: bool = False):
    ...
    if not st.session_state.get("us_signal_frozen") or st.session_state.get("us_signal_session_open"):
        ...
        # **이름은 「당일」 그대로 둔다** (2026-09-18 상하님 지시 — "장 시작 시 열고
        # 장 종료 시 당일 마감하고, 그게 다음날 장 시작 시 전일로 넘어가면 되지 않냐").
        # 이 칸은 이제 '방금 끝난 장이 마감된 채로 서 있는 자리'다. 그래서 날짜 뒤에
        # 「(마감)」을 붙인다 — 위 갈래의 「(장중)」·「(개장 전)」과 같은 자리다.
        # 예전 이름 「직전 미국장」은 뺐다(2026-08-12에 붙인 이름인데, 그때는 이 칸이
        # 「당일」이 아니라 '얼려 둔 값'이라는 뜻이었다).
        current_label_text = f"당일{' · ' + day if day else ''} (마감)"
