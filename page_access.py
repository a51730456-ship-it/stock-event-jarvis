...
#
# **2026-09-08 상하님 지시로 셋을 도로 열었다** — "한국테마는 다시 살려라" ·
# "자비스6 미국테마도 다시 살려라". 하루 전(2026-09-07)에 "미국테마 하나만
# 남겨라" 하신 것을 되돌린 것이다.
#
# 이름을 여기 넣으면 세 가지가 **한꺼번에** 살아난다. 다른 파일은 안 고쳐도 된다.
#   ① 「어디로 갈까요」 목록에 다시 오른다 — app.py 가 이 목록을 걸러 쓴다.
#   ② 주소로 바로 들어가도 안 막힌다.
#   ③ 미국테마 맨 위의 「🌏 한국테마 →」 단추가 다시 그려진다
#      (method_help.cross_link 는 갈 곳이 닫혀 있을 때만 안 그린다).
#
# 자비스6미국테마는 여기에 더하는 것만으로는 안 열렸다 — 그 화면에는 「수리
# 중입니다」 한 장만 그리고 멈추는 장치가 따로 있었다(2026-09-07). 그것도 같이
# 뗐다(pages/6_자비스6_미국테마.py).
#
# **2026-09-11 상하님 지시로 자비스6미국테마를 도로 닫았다** — 「어디로 갈까요」
# 캡처에서 그 줄에 동그라미를 치시고 "자비스6 온라인 화면에서 안 보이게 해라".
# 이름만 빼면 ①목록에서 사라지고 ②주소로 들어와도 막힌다. 되살리실 때는
# 여기에 "자비스6미국테마"를 도로 넣고 pages/6_자비스6_미국테마.py 의
# page_access.guard 한 줄을 빼면 된다. 파일은 하나도 안 지웠다.
OPEN_PAGES = ("미국테마", "한국테마", "자비스7", "자비스8")
